...
THEMES = {
    ...
    # ══════════════════════════════════════════════════════════════════════
    # 10 NEW TEMPLATE CATEGORIES
    # ══════════════════════════════════════════════════════════════════════

    # T1. Modern Bold — tech, marketing, startups
    "modern_bold": {
        "name": "Modern Bold",
        "primary":   HexColor("#0f172a"),
    ...
    # T2. Traditional Classic — law, finance, academia
    "traditional_classic": {
        "name": "Traditional Classic",
        "primary":   HexColor("#000000"),
        "accent":    HexColor("#1a1a1a"),
        "accent2":   HexColor("#555555"),
        "light":     HexColor("#ffffff"),
        "text":      HexColor("#000000"),
        "muted":     HexColor("#555555"),
        "border":    HexColor("#888888"),
        "header_bg": HexColor("#ffffff"),
        "header_fg": HexColor("#000000"),
        "layout":    "traditional",
    },
    # T3. Minimalist Clean — engineers, data analysts
    "minimalist_clean": {
        "name": "Minimalist Clean",
        "primary":   HexColor("#18181b"),
        "accent":    HexColor("#52525b"),
        "accent2":   HexColor("#a1a1aa"),
        "light":     HexColor("#fafafa"),
        "text":      HexColor("#18181b"),
        "muted":     HexColor("#71717a"),
        "border":    HexColor("#e4e4e7"),
        "header_bg": HexColor("#fafafa"),
        "header_fg": HexColor("#18181b"),
        "layout":    "minimalist",
    },
    # T4. Creative Vivid — designers, artists, copywriters
    "creative_vivid": {
        "name": "Creative Vivid",
        "primary":   HexColor("#4c1d95"),
        "accent":    HexColor("#ec4899"),
        "accent2":   HexColor("#fde68a"),
        "light":     HexColor("#fdf4ff"),
        "text":      HexColor("#1e1b4b"),
        "muted":     HexColor("#9333ea"),
        "border":    HexColor("#f0abfc"),
        "header_bg": HexColor("#4c1d95"),
        "header_fg": HexColor("#ffffff"),
        "sidebar_bg": HexColor("#4c1d95"),
        "sidebar_fg": HexColor("#ffffff"),
        "layout":    "creative",
    },
    # T5. Professional Navy — corporate, management, HR
    "professional_navy": {
        "name": "Professional Navy",
        "primary":   HexColor("#1e3a5f"),
        "accent":    HexColor("#1d4ed8"),
        "accent2":   HexColor("#93c5fd"),
        "light":     HexColor("#f0f7ff"),
        "text":      HexColor("#1e293b"),
        "muted":     HexColor("#4b6a9b"),
        "border":    HexColor("#bfdbfe"),
        "header_bg": HexColor("#1e3a5f"),
        "header_fg": HexColor("#ffffff"),
        "layout":    "professional",
    },
    # T6. Executive Elite — CEOs, directors, VPs
    "executive_elite": {
        "name": "Executive Elite",
        "primary":   HexColor("#0c0a09"),
        "accent":    HexColor("#b45309"),
        "accent2":   HexColor("#fcd34d"),
        "light":     HexColor("#fffbeb"),
        "text":      HexColor("#0c0a09"),
        "muted":     HexColor("#78350f"),
        "border":    HexColor("#fde68a"),
        "header_bg": HexColor("#0c0a09"),
        "header_fg": HexColor("#fcd34d"),
        "layout":    "executive",
    },
    # T7. Academic CV — professors, scientists, researchers
    "academic_cv": {
        "name": "Academic CV",
        "primary":   HexColor("#1e3a5f"),
    ...
    # T8. Entry Level Fresh — students, recent grads
    "entry_level_fresh": {
        ...
        "accent2":   HexColor("#6ee7b7"),
        "light":     HexColor("#ecfdf5"),
        "text":      HexColor("#064e3b"),
        "muted":     HexColor("#059669"),
        "border":    HexColor("#a7f3d0"),
        "header_bg": HexColor("#065f46"),
        "header_fg": HexColor("#ffffff"),
        "layout":    "entry_level",
    },
    # T9. Functional Skill-First — career changers, gaps
    "functional_skill": {
        "name": "Functional Skill-First",
        "primary":   HexColor("#312e81"),
        "accent":    HexColor("#7c3aed"),
        "accent2":   HexColor("#ddd6fe"),
        "light":     HexColor("#f5f3ff"),
        "text":      HexColor("#1e1b4b"),
        "muted":     HexColor("#6d28d9"),
        "border":    HexColor("#c4b5fd"),
        "header_bg": HexColor("#312e81"),
        "header_fg": HexColor("#ffffff"),
        "layout":    "functional",
    },
    # T10. Technical Grid — IT, developers
    "technical_grid": {
        "name": "Technical Grid",
        "primary":   HexColor("#0c4a6e"),
...
class ResumeGenerator:
    ...
    def generate(self, resume_data, output_path):
        dispatch = {
            "two_column":   self._generate_two_column,
            "classic":      self._generate_classic,
            "minimal":      self._generate_minimal,
            "modern_bold":  self._generate_modern_bold,
            "traditional":  self._generate_traditional,
            "minimalist":   self._generate_minimalist,
            ...
            "entry_level":  self._generate_entry_level,
            "functional":   self._generate_functional,
            "technical":    self._generate_technical,
        }
        fn = dispatch.get(self.layout, self._generate_standard)
        return fn(resume_data, output_path)
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # NEW LAYOUT 5 – MODERN BOLD (two-column, bold headers, tech/startup)
    # ══════════════════════════════════════════════════════════════════════════
    def _generate_modern_bold(self, data, output_path):
        """Bold left accent bar, two-tone header, chunky section titles."""
        T = self.T
        doc = SimpleDocTemplate(output_path, pagesize=A4,
            rightMargin=self.margin, leftMargin=self.margin,
            topMargin=0, bottomMargin=self.margin)
        styles = self._styles()
        # Override section title to be bolder
        styles["section_title"] = ParagraphStyle("section_title_mb",
            fontName="Helvetica-Bold", fontSize=11, textColor=T["accent"],
        ...
        role = personal.get("desired_role", "")
        contacts = []
        for icon, key in [("✉","email"),("✆","phone"),("⌖","location"),("in","linkedin"),("⌥","github")]:
            v = personal.get(key,"")
            if v: contacts.append(f"{icon} {v}")

        w = self.page_width - 2*self.margin
        # Header: name on left, accent bar on right
        name_style = ParagraphStyle("mb_name", fontName="Helvetica-Bold", fontSize=28,
            textColor=T["header_fg"], leading=32, spaceAfter=0)
        role_style = ParagraphStyle("mb_role", fontName="Helvetica", fontSize=12,
            textColor=T["accent2"], leading=15)
        contact_style = ParagraphStyle("mb_contact", fontName="Helvetica", fontSize=8.5,
            textColor=HexColor("#cbd5e1"), leading=12)

        hdr = Table([
            [Paragraph(name, name_style)],
            [Paragraph(role, role_style)] if role else [],
            [Paragraph("   |   ".join(contacts), contact_style)] if contacts else [],
        ], colWidths=[w])
        ...
        story.append(HRFlowable(width="100%", thickness=4, color=T["accent"], spaceAfter=8))
        story.extend(self._build_summary_section(data, styles))
        # Two-column body: skills | experience
        skill_col = self._build_skills_section(data, styles) + self._build_certs_section(data, styles) + self._build_languages_section(data, styles)
        main_col  = self._build_experience_section(data, styles) + self._build_projects_section(data, styles) + self._build_education_section(data, styles) + self._build_achievements_section(data, styles)
        col_w = (w - 6*mm) / 2
        body_tbl = Table([[skill_col or [Spacer(1,1)], main_col or [Spacer(1,1)]]], colWidths=[col_w, col_w + 6*mm])
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # NEW LAYOUT 6 – TRADITIONAL (centered name, horizontal rules, black/white)
    # ══════════════════════════════════════════════════════════════════════════
    def _generate_traditional(self, data, output_path):
        T = self.T
        doc = SimpleDocTemplate(output_path, pagesize=A4,
            rightMargin=self.margin, leftMargin=self.margin,
            topMargin=self.margin, bottomMargin=self.margin)
        ...
        # Centered name + role + contacts
        story.append(Paragraph(personal.get("name","Your Name"),
            ParagraphStyle("trad_name", fontName="Helvetica-Bold", fontSize=22,
                textColor=T["primary"], alignment=TA_CENTER, spaceAfter=2)))
        if personal.get("desired_role"):
            story.append(Paragraph(personal["desired_role"],
                ParagraphStyle("trad_role", fontName="Helvetica", fontSize=11,
                    textColor=T["accent2"], alignment=TA_CENTER, spaceAfter=4)))
        parts = [v for k in ["email","phone","location","linkedin","github"] if (v:=personal.get(k,""))]
        if parts:
            story.append(Paragraph("  |  ".join(parts),
                ParagraphStyle("trad_contact", fontName="Helvetica", fontSize=9,
                    textColor=T["muted"], alignment=TA_CENTER, spaceAfter=6)))
        story.append(HRFlowable(width="100%", thickness=2, color=T["primary"], spaceAfter=8))

        # All sections with ruled dividers
        for builder in [
            lambda: self._build_summary_section(data, styles),
            lambda: self._build_experience_section(data, styles),
            lambda: self._build_education_section(data, styles),
            lambda: self._build_skills_section(data, styles),
            lambda: self._build_projects_section(data, styles),
            lambda: self._build_certs_section(data, styles),
            lambda: self._build_achievements_section(data, styles),
        ]:
            items = builder()
            if len(items) > 1:
                story.extend(items)
                story.append(HRFlowable(width="100%", thickness=0.5, color=T["border"], spaceAfter=4))

        doc.build(story, onFirstPage=self._page_frame_simple, onLaterPages=self._page_frame_simple)
        return output_path

    # ══════════════════════════════════════════════════════════════════════════
    # NEW LAYOUT 7 – MINIMALIST (generous whitespace, single font, no decoration)
    # ══════════════════════════════════════════════════════════════════════════
    def _generate_minimalist(self, data, output_path):
        T = self.T
        doc = SimpleDocTemplate(output_path, pagesize=A4,
            rightMargin=28*mm, leftMargin=28*mm,
        ...
        if personal.get("desired_role"):
            story.append(Paragraph(personal["desired_role"],
                ParagraphStyle("min_role", fontName="Helvetica", fontSize=13,
                    textColor=T["muted"], spaceAfter=4, leading=16)))
        parts = [v for k in ["email","phone","location","linkedin","github"] if (v:=personal.get(k,""))]
        if parts:
            story.append(Paragraph("   ·   ".join(parts), styles["contact"]))
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # NEW LAYOUT 8 – CREATIVE (sidebar + vivid accent, unique geometry)
    # ══════════════════════════════════════════════════════════════════════════
    def _generate_creative(self, data, output_path):
        """Sidebar layout with vivid colors and decorative accents."""
        return self._generate_two_column(data, output_path)  # uses sidebar_bg/fg from theme

    # ══════════════════════════════════════════════════════════════════════════
    # NEW LAYOUT 9 – PROFESSIONAL (balanced, conservative accent, clear hierarchy)
    # ══════════════════════════════════════════════════════════════════════════
    def _generate_professional(self, data, output_path):
        T = self.T
        doc = SimpleDocTemplate(output_path, pagesize=A4,
            rightMargin=self.margin, leftMargin=self.margin,
            topMargin=0, bottomMargin=self.margin)
        ...
        story.extend(self._build_header_band(data, styles))
        story.extend(self._build_summary_section(data, styles))
        # Two balanced columns for skills + certs side by side
        w = self.page_width - 2*self.margin
        half = (w - 6*mm) / 2
        skills_items  = self._build_skills_section(data, styles)
        certs_items   = self._build_certs_section(data, styles)
        if skills_items and certs_items:
            side_tbl = Table([[skills_items, certs_items]], colWidths=[half, half])
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # NEW LAYOUT 10 – EXECUTIVE (dense, metrics-focused, expanded profile)
    # ══════════════════════════════════════════════════════════════════════════
    def _generate_executive(self, data, output_path):
        T = self.T
        doc = SimpleDocTemplate(output_path, pagesize=A4,
            rightMargin=self.margin, leftMargin=self.margin,
            topMargin=0, bottomMargin=self.margin)
        styles = self._styles()
        # Larger name, gold accent
        styles["name"] = ParagraphStyle("exec_name", fontName="Helvetica-Bold", fontSize=30,
            textColor=T["header_fg"], spaceAfter=2, leading=34)
        styles["tagline"] = ParagraphStyle("exec_tag", fontName="Helvetica", fontSize=11,
            textColor=T["accent2"], spaceAfter=4, leading=14)
        story = []
        story.extend(self._build_header_band(data, styles))
        # Executive Profile — expanded summary box
        summary = data.get("summary","")
        if summary:
            w = self.page_width - 2*self.margin
            summ_box = Table([[Paragraph(summary,
                ParagraphStyle("exec_summ", fontName="Helvetica", fontSize=10,
                    textColor=T["text"], leading=15, alignment=TA_JUSTIFY))]],
                colWidths=[w])
            summ_box.setStyle(TableStyle([
                ("BACKGROUND",(0,0),(-1,-1), T["light"]),
                ("LINEABOVE",(0,0),(-1,0), 2, T["accent"]),
                ("LINEBELOW",(0,0),(-1,-1), 2, T["accent"]),
                ("TOPPADDING",(0,0),(-1,-1),10),
        ...
        # Core competencies grid (skills as 3-col table)
        skills = data.get("skills",[])
        if skills:
            story.extend(self._section_header("Core Competencies", styles))
            w = self.page_width - 2*self.margin
            cols = 3
            col_w = w / cols
            rows, row = [], []
            for sk in skills:
                row.append(Paragraph(f"◆  {sk}", ParagraphStyle("exec_sk",
                    fontName="Helvetica-Bold", fontSize=9, textColor=T["text"], leading=12)))
                if len(row) == cols:
                    rows.append(row); row = []
            if row:
                while len(row) < cols: row.append(Paragraph("", styles["body"]))
                rows.append(row)
            if rows:
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # NEW LAYOUT 11 – ACADEMIC CV (multi-page, publications, research sections)
    # ══════════════════════════════════════════════════════════════════════════
    def _generate_academic(self, data, output_path):
        T = self.T
        doc = SimpleDocTemplate(output_path, pagesize=A4,
            rightMargin=self.margin, leftMargin=self.margin,
            topMargin=self.margin, bottomMargin=self.margin)
        ...
        story.append(Paragraph(personal.get("name","Your Name"),
            ParagraphStyle("acad_name", fontName="Helvetica-Bold", fontSize=20,
                textColor=T["primary"], alignment=TA_CENTER, spaceAfter=2)))
        if personal.get("desired_role"):
            story.append(Paragraph(personal["desired_role"],
                ParagraphStyle("acad_title", fontName="Helvetica-Oblique", fontSize=12,
                    textColor=T["muted"], alignment=TA_CENTER, spaceAfter=3)))
        parts = [v for k in ["email","phone","location","linkedin","github"] if (v:=personal.get(k,""))]
        if parts:
            story.append(Paragraph("   |   ".join(parts),
                ParagraphStyle("acad_contact", fontName="Helvetica", fontSize=9,
                    textColor=T["muted"], alignment=TA_CENTER, spaceAfter=6)))
        story.append(HRFlowable(width="100%", thickness=1.5, color=T["primary"], spaceAfter=10))

        # Research Summary / Profile
        story.extend(self._build_summary_section(data, styles))
        # Education first (most important for academic CV)
        story.extend(self._build_education_section(data, styles))
        story.extend(self._build_experience_section(data, styles))
        story.extend(self._build_projects_section(data, styles))
        story.extend(self._build_skills_section(data, styles))
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # NEW LAYOUT 12 – ENTRY LEVEL (large education, expanded skills, coursework)
    # ══════════════════════════════════════════════════════════════════════════
    def _generate_entry_level(self, data, output_path):
        T = self.T
        doc = SimpleDocTemplate(output_path, pagesize=A4,
            rightMargin=self.margin, leftMargin=self.margin,
            topMargin=0, bottomMargin=self.margin)
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # NEW LAYOUT 13 – FUNCTIONAL (grouped by capability, de-emphasised timeline)
    # ══════════════════════════════════════════════════════════════════════════
    def _generate_functional(self, data, output_path):
        T = self.T
        doc = SimpleDocTemplate(output_path, pagesize=A4,
            rightMargin=self.margin, leftMargin=self.margin,
            topMargin=0, bottomMargin=self.margin)
        ...
        story.extend(self._build_header_band(data, styles))
        story.extend(self._build_summary_section(data, styles))
        # Skills FIRST and centre-stage
        story.extend(self._build_skills_section(data, styles))
        # Key Achievements before work history
        story.extend(self._build_achievements_section(data, styles))
        story.extend(self._build_projects_section(data, styles))
        story.extend(self._build_certs_section(data, styles))
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # NEW LAYOUT 14 – TECHNICAL (dense, competency grid, prominent certs)
    # ══════════════════════════════════════════════════════════════════════════
    def _generate_technical(self, data, output_path):
        T = self.T
        doc = SimpleDocTemplate(output_path, pagesize=A4,
            rightMargin=self.margin, leftMargin=self.margin,
            topMargin=0, bottomMargin=self.margin)
        ...
        styles["bullet"] = ParagraphStyle("tech_bullet", fontName="Helvetica",
            fontSize=9, textColor=T["text"], leftIndent=12, spaceAfter=1, leading=11)
        story = []
        story.extend(self._build_header_band(data, styles))
        story.extend(self._build_summary_section(data, styles))

        # Core Competency Grid — 5 columns for density
        skills = data.get("skills", [])
        if skills:
            story.extend(self._section_header("Core Competencies", styles))
            w = self.page_width - 2*self.margin
            ...
            rows, row = [], []
            for sk in skills:
                row.append(Paragraph(sk, ParagraphStyle("tech_sk",
                    fontName="Helvetica", fontSize=8.5, textColor=T["text"],
        ...
        # Certs prominently before experience
        story.extend(self._build_certs_section(data, styles))
        story.extend(self._build_experience_section(data, styles))
        story.extend(self._build_projects_section(data, styles))
        story.extend(self._build_education_section(data, styles))
